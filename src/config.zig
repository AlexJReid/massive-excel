...
//
// Search order (first file that parses wins):
//   Windows XLL: 1. <dir containing the XLL>\config.json
//                2. %APPDATA%\zigxll-massive\config.json
//   Native CLI : 1. ./config.json
//                2. ./src/config.json
//
// The `-Dmassive_host`/`-Dmassive_port`/`-Dmassive_path`/`-Dmassive_insecure`
// build options supply defaults for fields not present in the JSON — so an
// unconfigured install still works against the endpoint baked in at build time.
//
// API key precedence:
...
const APPDATA_SUBDIR = "zigxll-massive";
...
// ----- Win32 bindings (hand-declared to avoid a cImport) --------------------
