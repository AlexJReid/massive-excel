...
- `src/ws_client.zig` — **hand-rolled TLS+WebSocket client** on top of
  `std.crypto.tls.Client` and `std.net`. ~487 lines. If you're touching
...
- `src/functions.zig` — all Excel UDFs. Exposes a generic `=MASSIVE(topic, [market])` escape hatch plus ~25 per-market typed wrappers (`MASSIVE.STOCKS.TRADE`, `MASSIVE.CRYPTO.TRADE`, etc.) built by a comptime `wrapper(meta)` helper. The ZigXLL framework discovers UDFs by walking top-level `pub` decls of this module.
...
// server data (can be an array if a batch, or single obj)
...
`zig build test` runs unit tests for `src/massive_protocol.zig` and `src/massive_rtd.zig` natively. Per-file tests also work directly: `zig test src/massive_protocol.zig`.
